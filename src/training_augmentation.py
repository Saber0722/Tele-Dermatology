...
# --------------------------------------------------
# Load Label Map (Needed for mel index)
# --------------------------------------------------
...
# --------------------------------------------------
# Dataset with Class-Conditional Augmentation
# --------------------------------------------------

class HAMDataset(Dataset):
    def __init__(self, csv_path, base_transform=None, mel_transform=None):
    ...
    def __getitem__(self, idx):
        ...
        # Apply stronger augmentation only to melanoma
        if label == MEL_INDEX and self.mel_transform is not None:
            image = self.mel_transform(image)
        else:
        ...
        return image, metadata, torch.tensor(label).long()
...
# Stronger transform for melanoma only
...
# Increase melanoma weight
class_weights[MEL_INDEX] *= 1.5
...
for epoch in range(15):
    ...
    if val_f1 > best_f1:
        best_f1 = val_f1
        torch.save(model.state_dict(), DATA_DIR / "best_model_augmented.pth")
        print("Saved best augmented model.")
